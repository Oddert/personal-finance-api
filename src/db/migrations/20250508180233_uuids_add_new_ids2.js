...
exports.up = function(knex) {
    return knex.schema.alterTable('budget', (table) => {
        ...
        table.uuid('category_id').notNullable().references('category.id')
        table.uuid('card_id').notNullable().references('card.id')
        ...
        table.uuid('scenario_id').notNullable().references('scenario.id')
    })).then(() => knex.schema.alterTable('scheduler', (table) => {
        table.uuid('transactor_id').notNullable().references('transactor.id')
    }))
};
